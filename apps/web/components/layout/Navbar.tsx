    ...
    <header className="sticky top-0 z-50 border-b border-white/5 bg-[#0a0a0a]/80 backdrop-blur-md">
      <div className="mx-auto flex h-16 max-w-6xl items-center justify-between px-4">
        <Link href="/" className="flex items-center gap-2 font-semibold text-white">
          <span className="flex h-7 w-7 items-center justify-center rounded-md bg-primary text-xs font-bold">B</span>
          <span className="text-base tracking-tight">Borderless</span>
        </Link>
        ...
        >
          <span className={`h-px w-5 bg-white transition-all ${open ? 'translate-y-[4px] rotate-45' : ''}`} />
          <span className={`h-px w-5 bg-white transition-all ${open ? 'opacity-0' : ''}`} />
          <span className={`h-px w-5 bg-white transition-all ${open ? '-translate-y-[8px] -rotate-45' : ''}`} />
        </button>
      </div>
      ...
      {open && (
        <div className="border-t border-white/5 bg-[#0a0a0a] px-4 py-4 md:hidden">
          ...
          <Link
            href="/download"
            onClick={() => setOpen(false)}
            className="mt-3 block rounded-md bg-primary px-4 py-2 text-center text-sm font-medium text-white"
          >
            Download
          </Link>
        </div>
