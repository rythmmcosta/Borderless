...
export default async function AdminDevicesPage() {
  ...
  return (
    <div>
      ...
      <Table>
        <THead><Tr><Th>Name</Th><Th>Platform</Th><Th>Owner</Th><Th>Status</Th><Th>Last Seen</Th></Tr></THead>
        <TBody>
          {devices.map((d) => (
            <Tr key={d.id}>
              <Td>{d.name}</Td>
              ...
              <Td className="text-gray-400 text-sm">{d.last_seen_at ? new Date(d.last_seen_at).toLocaleString() : '—'}</Td>
            </Tr>
          ))}
          {devices.length === 0 && <Tr><Td colSpan={5} className="text-center text-gray-500 py-8">No devices</Td></Tr>}
        </TBody>
      </Table>
    </div>
  )
