...
//!
//! Each device gets:
//!   - A stable UUID (derived from hardware fingerprint)
//!   - An Ed25519 keypair   (signing)
//!   - An X25519 keypair    (key exchange)
//!
...
impl DeviceIdentity {
    ...
    pub fn dh_public(&self) -> X25519Public { X25519Public::from(&self.dh_secret) }
    ...
    pub fn derive_session_key(&self, peer_dh_public: &X25519Public) -> [u8; 32] {
        ...
        let mut okm = [0u8; 32];
        hk.expand(b"borderless-v1-session-key", &mut okm).expect("HKDF length valid");
        okm
    }
    ...
    fn save(&self, path: &PathBuf) -> Result<(), CoreError> {
        ...
        let json = serde_json::to_string_pretty(&stored).map_err(CoreError::Serde)?;
    ...
    fn load(path: &PathBuf) -> Result<Self, CoreError> {
        ...
        let s: StoredIdentity = serde_json::from_str(&json).map_err(CoreError::Serde)?;
        let sign_bytes = hex::decode(&s.signing_bytes).map_err(|e| CoreError::Identity(e.to_string()))?;
        let sign_arr: [u8; 32] = sign_bytes.try_into().map_err(|_| CoreError::Identity("Bad signing key length".into()))?;
        let dh_bytes = hex::decode(&s.dh_bytes).map_err(|e| CoreError::Identity(e.to_string()))?;
        let dh_arr: [u8; 32] = dh_bytes.try_into().map_err(|_| CoreError::Identity("Bad DH key length".into()))?;
        Ok(Self {
            id:           s.id,
...
fn hardware_fingerprint() -> String {
    ...
    if let Ok(name) = hostname::get() { h.update(name.to_string_lossy().as_bytes()); }
    #[cfg(target_os = "linux")] {
        if let Ok(mid) = std::fs::read_to_string("/etc/machine-id") { h.update(mid.trim().as_bytes()); }
        else if let Ok(mid) = std::fs::read_to_string("/var/lib/dbus/machine-id") { h.update(mid.trim().as_bytes()); }
    }
    #[cfg(target_os = "macos")] {
        if let Ok(out) = std::process::Command::new("ioreg").args(["-rd1", "-c", "IOPlatformExpertDevice"]).output() { h.update(&out.stdout); }
    }
    #[cfg(target_os = "windows")] {
        if let Ok(out) = std::process::Command::new("reg").args(["query", "HKLM\\SOFTWARE\\Microsoft\\Cryptography", "/v", "MachineGuid"]).output() { h.update(&out.stdout); }
    }
    format!("{:x}", h.finalize())
}
